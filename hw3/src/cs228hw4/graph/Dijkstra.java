 ...
 * have a one-parameter constructor that accepts a reference to a DiGraph<V>. In
 * reality, the algorithm works on undirected graphs as well, but those are just
 * a specialization of DiGraph where each edge added is added in both directions
 * with the same cost each way. Each instance of Dijkstra<V> should be able to
 * handle calculating the shortest path from any vertex in the given graph to
 * any other vertex, as well as the corresponding shortest "distances". A client
 * to this interface will construct the concrete implementation with the graph
 * in question, then call run(V) to set the start vertex, then query for
 * shortest paths/distances from that start vertex. The client can then call
 * run(V) again to set a new start node and repeat.
 * 
 * @author Justin Mason
 *
