 ...
 * parameters). You may assume setColor() and setOpponentColor() are called only
 * once and before your Agent is used - these would be parameters for your
 * constructor, but for technical reasons, this cannot be done.
 */
public interface Agent {
	 ...
	 * GalaxyState to decide 3 actions for your agent. This version will only be
	 * used if this agent is run during a tournament.
	 * 
	 * @param g           a scan of the current state of the system
	 * @param energyLevel energy level corresponding to the amount of energy the
	 *                    agent has
	 * @return an array of 3 AgentActions that indicates the agent's next 3 actions
	 *         if in a tournament situation. If fewer than 3 actions are specified
	 *         or there are null actions, these agent moves are lost.
	 */
	 ...
	 * Function used to generate command turns from your agent. Use the passed
	 * GalaxyState to decide 3 actions for your agent. This version will only be
	 * used if this agent is run during grading.
	 * 
	 * @param g           a scan of the current state of the system
	 * @param energyLevel energy level corresponding to the amount of energy the
	 *                    agent has
	...
	/**
	 * Allow the simulation to set the color of your agent. It's up to you to save
	 * this information somehow. This method is called only once, at the start of
	 * each simulation.
	 * 
	...
	/**
	 * Allow the simulation to set the color of your agent's opponent. It's up to
	 * you to save this information if you need it. This method is called only once,
	...
	/**
	 * The fortify action of an agent. Requires the amount of energy spent on
	 * fortifying.
	 */
	public class Fortify implements AgentAction {
		...
		/**
		 * Get the amount of energy that will be spent fortifying in this fortify
		 * action.
		 * 
	...
	/**
	 * The setscan action of an agent. Requires amount of energy to spend during
	 * each scan.
	 */
	public class SetScan implements AgentAction {
		...
		/**
		 * Ctor that constructs a setscan action given the amount of energy to spend on
		 * scanning.
		 * 
	...
	public class Capture implements AgentAction {
		...
		/**
		 * Ctor that constructs a capture action that will use the given amount of
	...
	/**
	 * The shoot action of an agent. Requires energy spent on shot and path from the
	 * agent's current system (exclusive) to the target (inclusive).
	 */
	public class Shoot implements AgentAction {
		...
		/**
		 * Returns the path the ion torpedo is supposed to take. Note that this returns
		 * the actual path, not a copy.
		 * 
